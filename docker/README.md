...
| Path | Description |
|---|---|
| `oss-cad-suite/Dockerfile` | Builds the image from a pinned [oss-cad-suite](https://github.com/YosysHQ/oss-cad-suite-build) release. The release tag, date, and a SHA-256 of the downloaded archive are `ARG`s, so the toolchain release is pinned and its download is integrity-checked (fail-closed). The apt layer tracks rolling security updates, so a rebuild is not bit-identical; image identity is anchored on the built image's digest. Adds `tini` as PID 1 and a non-root `oneware` user. |
