...
public partial class DockerDiagnosticsView
{
    ...
    private async Task PopulateTelemetryAsync()
    {
        try
        {
            ...
            if (entries.Count > 0)
            {
                _telemetryContent.Children.Add(BuildHistoricalTrendsPanel(entries));
                ...
                {
                    _telemetryContent.Children.Add(BuildTelemetryEntryRow(entry));
                }
                ...
                _telemetryContent.Children.Add(BuildTelemetryActionsRow());
            }
            else if (totalRuns == 0)
            {
                _telemetryContent.Children.Add(new TextBlock
    ...
    private static IEnumerable<ContainerTelemetry.TelemetryEntry> SortTelemetryEntries(List<ContainerTelemetry.TelemetryEntry> entries, (string column, bool ascending) sort)
    {
        return sort.column switch
        {
            ...
            _ => sort.ascending // "time"
                ? entries.OrderBy(e => e.Timestamp, StringComparer.Ordinal)
                : entries.OrderByDescending(e => e.Timestamp, StringComparer.Ordinal),
        };
    }
    ...
    private Grid BuildTelemetryEntryRow(ContainerTelemetry.TelemetryEntry entry)
    {
        ...
        var timeLabel = "";
        if (DateTime.TryParse(entry.Timestamp, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out var ts))
            ...
            pinBtn = new Button
            {
                ...
                Command = new AsyncRelayCommand(async () =>
                {
                    try
                    {
                        ...
                        {
                            await topLevel.Clipboard.SetTextAsync(pinRef).ConfigureAwait(false);
            ...
            copyBtn = new Button
            {
                ...
                Command = new AsyncRelayCommand(async () =>
                {
                    try
                    {
                        ...
                        if (topLevel?.Clipboard != null)
                        {
                            await topLevel.Clipboard.SetTextAsync(cmdText).ConfigureAwait(false);
